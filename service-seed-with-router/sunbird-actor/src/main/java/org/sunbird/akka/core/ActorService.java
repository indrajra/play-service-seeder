...
public class ActorService {
	...
	private Set<Class<?>> getActors(List<String> actorsClassPathList) {
		  synchronized (ActorService.class) { Reflections reflections = null;
		  Set<Class<?>> actors = new HashSet<>(); for(String classpath :
		  actorsClassPathList){ reflections = new Reflections(classpath);
		  actors.addAll(reflections.getTypesAnnotatedWith(ActorConfig.class)); } return
		  actors; }
    ...
    private void createActor(Class actor,
            String[] operations,
			String dispatcher) {
		  if (null != operations && operations.length > 0) { Props props; if
		  (StringUtils.isNotBlank(dispatcher)) { props =
		  Props.create(actor).withDispatcher(dispatcher); } else { props =
		  Props.create(actor); } ActorRef actorRef =
		  system.actorOf(FromConfig.getInstance().props(props), actor.getSimpleName());
		  for (String operation : operations) { actorRefCache.put(operation, actorRef);
		  } }
		 }
